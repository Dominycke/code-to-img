...
function CodeEditor({
  language, theme, icon, background, currentPadding
}: CodeEditorProps) {
  ...
  return (
    <Resizable minHeight={466} minWidth={510} maxWidth={1000} defaultSize={{
      width: width,
      height: height || 500,
    }}
      ...
      <div className='code-block'
        ...
        >
          ...
          <div className='input-control w-full'>
            <input type='text' value={title} onChange={(e) => setTitle(e.target.value)} className='w-full py-2 text-[hsla(0, 0%, 100%, .6)] outline-none font-medium text-center bg-transparent' />
          </div>

          <div className="icon flex justify-center items-center p-1 bg-black bg-opacity-30 rounded-sm">
            {/* {icon} */}
            <img src={icon} alt="icon" className='w-[33px] h-[33px]' />
          </div>
        </div>
        <AceEditor
          value={code}
